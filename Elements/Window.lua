-- Elements/Window.lua
-- Enhanced window element with shadow toggling
...
function Window.new(title, x, y, width, height, options)
    ...
    if _G.CensuraG.Config.EnableShadows then
        shadow = Utilities.createTaperedShadow(frame, 5, 5, 0.9)
    ...
    -- Shadow toggling event listener
    table.insert(self.Connections, EventManager:SubscribeToEvent("ShadowsToggled", function(state)
        if state and not self.Shadow then
            self.Shadow = Utilities.createTaperedShadow(self.Instance, 5, 5, 0.9)
...
function Window:Minimize()
    ...
    if self.Shadow then Animation:SlideY(self.Shadow, screenHeight+45, 0.3) end
...
function Window:Restore(callback)
    ...
    if self.Minimized then
        ...
        if self.Shadow then Animation:SlideY(self.Shadow, self.CurrentPosition.Y.Offset-5, 0.3) end
...
function Window:MakeResizable()
    ...
    for i, handle in ipairs(handles) do
        ...
        handle.MouseMoved:Connect(function(x,y)
            ...
            if self.Shadow then self.Shadow.Size = UDim2.new(0, newSize.X.Offset+10, 0, newSize.Y.Offset+10) end
...
function Window:Destroy()
    ...
    for _, conn in ipairs(self.Connections) do
        EventManager:UnsubscribeFromEvent("ShadowsToggled", conn)
    end
    self.Connections = {}
