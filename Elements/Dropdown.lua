...
function Dropdown.new(parent, x, y, width, options, defaultSelection, callback)
	...
	dropButton.MouseEnter:Connect(function()
		Animation:Tween(dropButton, { BackgroundTransparency = Styling.Transparency.ElementBackground - 0.1 }, 0.1)
	end)
	dropButton.MouseLeave:Connect(function()
		Animation:Tween(dropButton, { BackgroundTransparency = Styling.Transparency.ElementBackground }, 0.1)
	end)
	...
	local self = setmetatable({
		Instance = frame, Label = label, Button = dropButton, Icon = dropIcon,
		List = dropList, Items = items, SelectedItem = selectedText, IsOpen = false,
	...
	function self:PopulateList()
		...
		for i, item in ipairs(self.Items) do
			...
			itemButton.MouseEnter:Connect(function()
				Animation:Tween(itemButton, { BackgroundTransparency = Styling.Transparency.ElementBackground - 0.1 }, 0.1)
			end)
			itemButton.MouseLeave:Connect(function()
				Animation:Tween(itemButton, { BackgroundTransparency = Styling.Transparency.ElementBackground }, 0.1)
			end)
	...
	function self:ToggleList(forceState)
		...
		if newState then
			...
			self.Icon.Text = "▲"
			Animation:Tween(self.List, { Size = UDim2.new(0, width-70, 0, #self.Items*30) }, 0.2)
			EventManager:FireEvent("DropdownOpened", self)
		else
			self.Icon.Text = "▼"
			Animation:Tween(self.List, { Size = UDim2.new(0, width-70, 0, 0) }, 0.2, nil, nil, function()
	...
	table.insert(self.Connections, EventManager:Connect(UserInputService.InputBegan, function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 and self.IsOpen then
			...
			if not Utilities.isPointInElement(self.Button, pos) and
			   not Utilities.isPointInElement(self.List, pos) then
				self:ToggleList(false)
			end
		end
	end))
