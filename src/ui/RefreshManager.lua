-- CensuraG/src/ui/RefreshManager.lua (Improved version)
...
-- Enhanced type checking for component refresh
function RefreshManager:RefreshComponent(component, instance)
    -- Enhanced error checking for component and instance
    ...
    if not instance then
        _G.CensuraG.Logger:error("Nil instance provided for component: " .. component)
        return
    end
    ...
    if typeof(instance) == "table" then
        if instance.Instance then
            ...
            -- Try to refresh the table object directly
            targetInstance = instance
            _G.CensuraG.Logger:debug("Using table object directly for component: " .. component)
        end
    elseif typeof(instance) == "Instance" then
        targetInstance = instance
    else
        _G.CensuraG.Logger:error("Cannot refresh invalid component: " .. typeof(instance) .. ": " .. tostring(instance))
        return
    end
    ...
    -- Use pcall to prevent errors from breaking the refresh process
        ...
        if component == "window" then
            ...
            if typeof(instance) == "table" then
                ...
                if instance.ContentFrame then
                    ...
                    -- Also refresh all children in ContentFrame
        ...
        elseif component == "grid" then
            ...
            if typeof(instance) == "table" then
                if instance.Instance then
                    ...
                    -- Refresh all children in the grid
            ...
            -- For any other component types, try to refresh based on common properties
            _G.CensuraG.Logger:debug("Refreshing unknown component type: " .. component)
            
            if typeof(targetInstance) == "Instance" then
                -- Try to apply theme to common properties
                if targetInstance:IsA("GuiObject") then
                    if targetInstance:IsA("TextLabel") or targetInstance:IsA("TextButton") or targetInstance:IsA("TextBox") then
                        _G.CensuraG.AnimationManager:Tween(targetInstance, {
                            TextColor3 = theme.TextColor,
                            TextSize = theme.TextSize
                        }, animConfig.FadeDuration)
                    ...
                    if targetInstance:IsA("Frame") or targetInstance:IsA("TextButton") or targetInstance:IsA("TextBox") then
                        _G.CensuraG.AnimationManager:Tween(targetInstance, {
                            BackgroundColor3 = theme.SecondaryColor,
                            BackgroundTransparency = 0.8
                        }, animConfig.FadeDuration)
    ...
    if not success then
        _G.CensuraG.Logger:warn("Error while refreshing component " .. component .. ": " .. tostring(err))
    else
...
function RefreshManager:RefreshUIElement(element, theme)
    ...
    if not element:IsA("Instance") then return end
    ...
    if not success then
        _G.CensuraG.Logger:debug("Error refreshing element: " .. tostring(err))
    end
end
...
function RefreshManager:RefreshAll()
    ...
    -- Use pcall to prevent errors from breaking the entire refresh process
    pcall(function()
        ...
        if _G.CensuraG.Windows then
            for i, window in ipairs(_G.CensuraG.Windows) do
                if window and typeof(window) == "table" then
                    -- Check if window has a Refresh method
                    if typeof(window.Refresh) == "function" then
                        pcall(function() window:Refresh() end)
                    else
                        -- If no refresh method, refresh manually
        ...
        if _G.CensuraG.Taskbar and _G.CensuraG.Taskbar.Instance then
            if typeof(_G.CensuraG.Taskbar.Instance) == "table" and _G.CensuraG.Taskbar.Instance.Refresh then
                pcall(function() _G.CensuraG.Taskbar.Instance:Refresh() end)
            elseif _G.CensuraG.TaskbarManager and _G.CensuraG.TaskbarManager.Refresh then
                pcall(function() _G.CensuraG.TaskbarManager:Refresh() end)
            end
        end
    end)
